...
// For more on ANSI escape codes see https://en.wikipedia.org/wiki/ANSI_escape_code
// and https://en.wikipedia.org/wiki/ANSI_escape_code#Colors in particular.
...
func init() {
	...
	for color := DarkGray; color < numColors; color++ {
		fgColorBytes = append(fgColorBytes, []byte(
			fmt.Sprintf("\x1b[%d;1m", 30+color-DarkGray)))
		bgColorBytes = append(bgColorBytes, []byte(
			fmt.Sprintf("\x1b[%d;1m", 40+color-DarkGray)))
	}
}
